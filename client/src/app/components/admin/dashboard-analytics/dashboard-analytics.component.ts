...
import { FullAnalyticsResponse } from '../../../models/analtics.models';
...
export class DashboardAnalyticsComponent implements OnInit, AfterViewInit {
  @ViewChild('userGrowthChart')
  userGrowthChartRef!: ElementRef<HTMLCanvasElement>;
  @ViewChild('documentUploadsChart')
  documentUploadsChartRef!: ElementRef<HTMLCanvasElement>;
  @ViewChild('documentTypesChart')
  documentTypesChartRef!: ElementRef<HTMLCanvasElement>;
  ...
  createUserGrowthChart() {
    ...
    if (!this.analytics.userGrowth || this.analytics.userGrowth.length === 0)
      return;

    // Prepare data
    const labels = this.analytics.userGrowth.map(
      (item) => `${this.monthNames[item._id.month - 1]} ${item._id.year}`,
    );
  ...
  createDocumentUploadsChart() {
    ...
    // Prepare data
    const labels = this.analytics.documentUploads.map(
      (item) => `${this.monthNames[item._id.month - 1]} ${item._id.year}`,
    );
