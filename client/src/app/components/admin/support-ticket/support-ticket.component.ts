  ...
  async sendMessage() {
    ...
    try {
      ...
      if (!this.socketService.isConnected()) {
        ...
        if (!isConnected) {
          console.warn(
            'Failed to reconnect socket, message may not be delivered',
          );
          this.toastr.warning(
            'Connection issue detected, message may not be delivered',
            'Connection Warning',
          );
        }
      }
      ...
      if (!this.socketService.isInTicketRoom(ticketId)) {
        console.log(
          `Admin not in ticket room ${ticketId}, attempting to join before sending message`,
        );
        try {
          await this.socketService.joinTicketRoom(ticketId);
          console.log(
            `Admin successfully joined ticket room ${ticketId} before sending message`,
          );
        } catch (joinError) {
          console.error(
            `Admin failed to join ticket room ${ticketId} before sending message:`,
            joinError,
          );
