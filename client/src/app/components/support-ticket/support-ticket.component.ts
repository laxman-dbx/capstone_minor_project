...
export class SupportTicketComponent implements OnInit, OnDestroy {
    ...
    if (!this.isSocketConnected) {
      try {
          ...
          (isConnected) => {
            if (isConnected) {
              ...
              if (this.selectedTicketData && this.currentTicketId) {
                ...
                this.socketService
                  .joinTicketRoom(this.currentTicketId)
                  .then(() => {
                    console.log(
                      `Successfully joined ticket room ${this.currentTicketId} after connection`,
                    );
                    // Reload messages to ensure we have the latest
                    if (this.currentTicketId) {
                      return this.socketService.getChatHistory(
                  ...
                  .catch((err) =>
                    console.error('Error joining room after connection:', err),
                  );
              }
        ...
        connectionTimeout = setTimeout(() => {
          console.log(
            'Socket connection timeout - proceeding without real-time updates',
          );
          this.isSocketConnected = false;
          subscription.unsubscribe();
          ...
          this.toastr.warning(
            'Unable to establish real-time connection. Messages may not update automatically.',
            'Connection Issue',
          );
      ...
      this.socketService.listenToMessages().subscribe((message) => {
        this.ngZone.run(() => {
          ...
          if (
            ...
            console.log(
              'Message is for a different ticket or no ticket selected',
            );
      ...
      this.socketService.connectionStatus$.subscribe((isConnected) => {
        this.ngZone.run(() => {
          ...
          if (isConnected) {
            ...
            if (this.currentTicketId) {
              console.log(
                `Rejoining ticket room ${this.currentTicketId} after reconnection`,
              );
              this.socketService
                .joinTicketRoom(this.currentTicketId)
                .then(() => {
                  console.log(
                    `Successfully rejoined ticket room ${this.currentTicketId}`,
                  );
    ...
    try {
      ...
      for (const pendingMessage of messagesToSend) {
        try {
          ...
          console.log(
            `Successfully sent pending message: ${pendingMessage.message.substring(0, 20)}...`,
          );
          ...
          console.error(
            `Failed to send pending message: ${pendingMessage.message.substring(0, 20)}...`,
            error,
          );

          // Add back to pending messages
          const currentPendingMessages = JSON.parse(
            localStorage.getItem('pendingMessages') || '[]',
          );
          currentPendingMessages.push(pendingMessage);
          localStorage.setItem(
            'pendingMessages',
            JSON.stringify(currentPendingMessages),
          );
        }
      }
      ...
      if (messagesToSend.length > 0) {
        this.toastr.success(
          `Sent ${messagesToSend.length} pending messages`,
          'Sync Complete',
        );
      }
    } catch (error) {
  ...
  async createTicket() {
    ...
    try {
      this.loading = true;
      const response = await this.userTicketService.createTicket(
        this.newTicket,
      );
  ...
  async selectTicket(ticketId: string) {
    try {
      ...
      if (!isConnected) {
        ...
        try {
          ...
          if (isConnected) {
            ...
            console.warn(
              'Socket reconnection failed, continuing with limited functionality',
            );
            this.toastr.warning(
      ...
      try {
        ...
        this.toastr.warning(
          'Unable to connect to real-time updates for this ticket',
          'Connection Issue',
        );
      }
      ...
      try {
        ...
        if (joinedRoom && isConnected) {
          try {
            ...
            const retryHistory = await this.socketService.getChatHistory(
              ticket._id,
            );
  ...
  async sendMessage() {
    ...
    try {
      ...
      if (!this.isSocketConnected) {
        console.log(
          'Socket not connected, attempting to reconnect before sending message',
        );
        this.socketService.reconnect();
        ...
        if (!isConnected) {
          console.warn(
            'Failed to establish socket connection, storing message locally',
          );
          this.toastr.warning(
            'Message saved locally. It will be sent when connection is restored.',
            'Offline Mode',
          );
      ...
      if (!this.socketService.isInTicketRoom(ticketId)) {
        console.log(
          `Not in ticket room ${ticketId}, attempting to join before sending message`,
        );
        try {
          await this.socketService.joinTicketRoom(ticketId);
          console.log(
            `Successfully joined ticket room ${ticketId} before sending message`,
          );
        } catch (joinError) {
          console.error(
            `Failed to join ticket room ${ticketId} before sending message:`,
            joinError,
          );
      ...
      if (this.selectedTicketData?.messages) {
        const index = this.selectedTicketData.messages.findIndex(
          (m) =>
            ...
            m.sender === 'user' &&
            Math.abs(new Date().getTime() - new Date(m.timestamp).getTime()) <
              5000,
        );
