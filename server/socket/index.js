...
module.exports = (io) => {
  ...
  io.on("connection", (socket) => {
    ...
    socket.conn.on("upgrade", (transport) => {
      console.log(
        `Transport upgraded for user ${socket.user.id} to ${transport}`,
      );
    });
    ...
    socket.on("joinTicketRoom", async (data, callback) => {
      try {
        ...
        console.log(
          `User ${socket.user.id} attempting to join ticket room: ${ticketId}`,
        );
        ...
        if (typeof callback === "function")
          callback({ error: "Failed to join ticket room" });
      }
    });
    ...
    socket.on("sendMessage", async (data, callback) => {
      try {
        ...
        if (ticket.status === "resolved") {
          console.error(
            "Cannot send message to resolved ticket:",
            data.ticketId,
          );
        ...
        if (!ticketOwnerId) {
          console.error(
            "Cannot determine ticket owner for ticket:",
            data.ticketId,
          );
    ...
    socket.on("getChatHistory", async (data, callback) => {
      try {
        ...
        if (!isAdmin && !isTicketOwner) {
          console.log(
            "Chat history access denied: User is neither admin nor ticket owner",
          );
