    ...
    if (ParsedResults && ParsedResults.length > 0) {
      ...
      const piiPatterns = [
        ...
        { label: "Passport Number", regex: /[A-Z]{1}\d{7}/g },
        {
          label: "Date of Birth",
          regex:
            /\b(0[1-9]|1[0-9]|2[0-9]|3[01])[-/](0[1-9]|1[0-2])[-/]\d{4}\b/g,
        },
      ...
      piiPatterns.forEach((pattern) => {
        ...
        if (matches) {
          matches.forEach((match) => {
            const matchingIndex = ocrData.Lines.findIndex((line) =>
              line.LineText.includes(match),
            );
            let matchingLine;
            if (matchingIndex !== -1) {
            ...
            if (matchingLine) {
              if (pattern.label === "Aadhaar Number") {
                ...
                if (isdobMatch || isPan) {
                  const dobMatch =
                    /\b(0[1-9]|1[0-9]|2[0-9]|3[01])[-/](0[1-9]|1[0-2])[-/]\d{4}\b/g.exec(
                      match,
                    );
                  const dob = dobMatch[0];
                  const matchingWord = matchingLine.Words.find(
                    (word) => word.WordText === dob,
                  );
