...
function pdfToJpgConverter(sourceFile, pages = "", password = "") {
  return getPresignedUrl(API_KEY, sourceFile)
    .then(([uploadUrl, uploadedFileUrl]) =>
      uploadFile(sourceFile, uploadUrl).then(() => uploadedFileUrl),
    )
    .then((uploadedFileUrl) =>
      convertPDFToJPG(API_KEY, uploadedFileUrl, password, pages),
    )
    .catch(console.error);
}
    ...
    https
      .get(reqOptions, (response) => {
        ...
        response.on("end", () => {
          ...
          jsonResponse.error
            ? reject("Error getting presigned URL: " + jsonResponse.message)
            : resolve([jsonResponse.presignedUrl, jsonResponse.url]);
        });
      })
